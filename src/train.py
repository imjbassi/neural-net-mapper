...
def train_model(
    ...
    Saves:
        outputs/snapshots.npz containing a list of dict snapshots.
        Each snapshot has keys: epoch, acts, weights, loss_hist, acc_hist, img, label,
        pred, conf, hidden_sizes, dropout_masks, dropout_p, classes.
    """
    ...
    for epoch in range(1, epochs + 1):
        ...
        if epoch % sample_every == 0 or epoch == 1 or epoch == epochs:
            ...
            sample_idx = 0
            sample_x = torch.tensor(X_val[sample_idx], dtype=torch.float32).unsqueeze(0).to(device)
